...
object Springs {
    ...
    lazy val input: String = scala.io.Source.fromInputStream(getClass.getResourceAsStream("./input/springs_input")).mkString.trim
    ...
    private def parseRecords(input: String): Seq[Record] = input.linesIterator.map(parseRecord).toSeq

    private def parseRecord(s: String): Record = s match {
        case s"$mask $lengths" =>
    ...
    private def countArrangements(mask: List[Char], lengths: List[Int]): Long = {
